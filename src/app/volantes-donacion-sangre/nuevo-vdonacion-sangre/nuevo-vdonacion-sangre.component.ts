...
export class NuevoVdonacionSangreComponent implements OnInit {
  ...
  formNuevaDonacion: any = this.formBuilder.group({
    ...
    tipoSangre: new FormControl(
      ...
      Validators.compose([
        Validators.required,
        Validators.pattern('([AaBbOo]|[Aa][Bb])[+-]'),
      ]),
    ),
  ...
  ngOnInit(): void {
    ...
    if (this.paciente !== null && this.paciente !== undefined) {
      ...
      this.formNuevaDonacion.controls['desNSS'].setValue(
        nss + ' ' + nssAgregado,
      )
  ...
  registarDonacion(formNuevaDonacion: FormGroup) {
    ...
    if (formNuevaDonacion.status != 'INVALID') {
      ...
      if (
        this.formNuevaDonacion.value.codigoPostal.length == 0 ? true : false
      ) {
      ...
      if (
        this.formNuevaDonacion.value.nombreTrabajadorSocial.length == 0
          ? true
          : false
      ) {
      ...
      if (
        this.formNuevaDonacion.value.matriculaTrabajadorSocial.length == 0
          ? true
          : false
      ) {
        this.formNuevaDonacion.controls['matriculaTrabajadorSocial'].setValue(
      ...
      if (
        this.formNuevaDonacion.value.numTelefonoTrabajadorSocial.length == 0
          ? true
          : false
      ) {
        this.formNuevaDonacion.controls['numTelefonoTrabajadorSocial'].setValue(
          null,
        )
        this.muestraAlerta(
          'Verificar datos capturados',
      ...
      if (
        this.formNuevaDonacion.value.observaciones.length == 0 ? true : false
      ) {
        ...
        async (res: any) => {
          ...
          if (estatus == 'OK') {
            ...
            await this.router.navigateByUrl(
              '/detalle-volante-donacion-sangre/' + id,
            )
          } else {
            this.muestraAlerta(
  ...
  onChangeEstado(): void {
    this.estudioSocialService
      .getCatMunicipiosByEstado(this.formNuevaDonacion.get('idEstado').value)
      .toPromise()
      .then(
        (delegaciones: Municipio[]) => {
  ...
  onChangeMunicipio(): void {
    this.estudioSocialService
      .getCiudadByEstadoMunicipio(
        this.formNuevaDonacion.get('idEstado').value,
        this.formNuevaDonacion.get('idDelegacion').value,
      )
      .toPromise()
      .then(
        (ciudades: Ciudad[]) => {
          ...
          if (this.ciudadesFamiliar.length === 1) {
            this.formNuevaDonacion.controls['idCiudad'].setValue(
              this.ciudadesFamiliar[0].cve_ciudad,
            )
          }
        },
        (httpErrorResponse: HttpErrorResponse) => {
