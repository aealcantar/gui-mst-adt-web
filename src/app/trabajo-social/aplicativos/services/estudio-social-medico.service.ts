...
export class EstudioSocialMedicoService {
  ...
  getCatEstadosCiviles() {
    return this.http.get<EstadoCivil[]>(`${environment.urlServEstudioMedicos}/estudiosMedicos/getEstadosCiviles`);
  }
  ...
  getCatTiposComunidad() {
    return this.http.get<EstadoCivil[]>(`${environment.urlServEstudioMedicos}/estudiosMedicos/getTiposComunidad`);
  }

  getEstudiosMedicosByFechas(fechaInicio: string, fechaFin: string) {
    return this.http.get<EstudioMedico[]>(`${environment.urlServEstudioMedicos}/estudiosMedicos/findEstudiosMedByRangoFechas/${fechaInicio}/${fechaFin}`);
  }

  addEstudioSocial(estudioMedicoSocial: EstudioMedico){
    return this.http.post<EstudioMedico>(`${environment.urlServEstudioMedicos}/estudiosMedicos/guardaNuevoEstudioMed`, estudioMedicoSocial);
  }

  downloadPdf(data: any): Observable<Blob> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json', responseType: 'blob' });
    return this.http.post<Blob>(`${environment.urlServEstudioMedicos}/reporte/reporteEstudiosMedicos`, JSON.stringify(data),
