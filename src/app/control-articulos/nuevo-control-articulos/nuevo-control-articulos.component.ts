...
export class NuevoControlArticulosComponent implements OnInit, AfterViewInit {
  ...
  formNuevoArticulo: any = this.formBuilder.group({
    ...
    resguardoFecha: new FormControl('', [
      Validators.required,
      Validators.pattern(
        /(?:(?:31(\/|-|\.)(?:0?[13578]|1[02]))\1|(?:(?:29|30)(\/|-|\.)(?:0?[13-9]|1[0-2])\2))(?:(?:1[6-9]|[2-9]\d)?\d{2})$|^(?:29(\/)0?2\3(?:(?:(?:1[6-9]|[2-9]\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))$|^(?:0?[1-9]|1\d|2[0-8])(\/)(?:(?:0?[1-9])|(?:1[0-2]))\4(?:(?:1[6-9]|[2-9]\d)?\d{4})/,
      ),
    ...
    resguardoNombreEntrega: new FormControl('', [
      Validators.required,
      Validators.maxLength(150),
    ]),
    recepcionFecha: new FormControl('', [
      Validators.required,
      Validators.pattern(
        /(?:(?:31(\/|-|\.)(?:0?[13578]|1[02]))\1|(?:(?:29|30)(\/|-|\.)(?:0?[13-9]|1[0-2])\2))(?:(?:1[6-9]|[2-9]\d)?\d{2})$|^(?:29(\/)0?2\3(?:(?:(?:1[6-9]|[2-9]\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))$|^(?:0?[1-9]|1\d|2[0-8])(\/)(?:(?:0?[1-9])|(?:1[0-2]))\4(?:(?:1[6-9]|[2-9]\d)?\d{4})/,
      ),
    ...
    recepcionNombreRecibe: new FormControl('', [
      Validators.required,
      Validators.maxLength(150),
    ]),
    recepcionNombreEntrega: new FormControl('', [
      Validators.required,
      Validators.maxLength(150),
    ]),
    recepcionUbicacion: new FormControl('', Validators.required),
    recepcionHorarioEntregaArticulo: new FormControl('', [
      Validators.required,
  ...
  ngOnInit(): void {
    ...
    if (this.paciente !== null && this.paciente !== undefined) {
      ...
      this.formNuevoArticulo.controls['desAgregadoMedico'].setValue(
        desAgregadoMedico,
      )
    }
    ...
    if (userTmp !== '') {
      ...
      this.formNuevoArticulo.controls['personalQueElaboro'].setValue(
        'Roberto García',
      )
      this.formNuevoArticulo.controls['trabajadorNombreRecibe'].setValue(
        'Roberto García',
      )
      this.formNuevoArticulo.controls['resguardoNombreRecibe'].setValue(
        'Roberto García',
      )
    }
    ...
    this.formNuevoArticulo.controls['horarioEntregaArticulo'].setValue(
      this.hora,
    )
  ...
  guardarControl() {
    ...
    if (this.formNuevoArticulo.status != 'INVALID') {
      ...
      if (
        this.formNuevoArticulo.value.noCama.trim().length == 0 ? true : false
      ) {
      ...
      if (
        ...
        this.formNuevoArticulo.controls['enfermeriaNombreEntrega'].setValue(
          null,
        )
        this.muestraAlerta(
          'Verificar datos capturados',
      ...
      if (
        ...
        this.formNuevoArticulo.controls[
          'recepcionHorarioEntregaArticulo'
        ].setValue(null)
        this.muestraAlerta(
          'Verificar datos capturados',
          'alert-danger',
      ...
      let rangoFecha1 = this.formNuevoArticulo.controls[
        'recepcionHorarioEntregaArticulo'
      ].value.substring(0, 4)
      let rangoFecha2 = this.formNuevoArticulo.controls[
        'recepcionHorarioEntregaArticulo'
      ].value.substring(4, 8)
      ...
      this.formNuevoArticulo.controls[
        'recepcionHorarioEntregaArticulo'
      ].patchValue(`${rangoFecha1} - ${rangoFecha2}`)
      this.formNuevoArticulo.controls['']
  ...
  ajustarHora(formName: string) {
    const hora = moment(
      this.formNuevoArticulo.get(formName).value,
      'HHmm',
    ).format('HH:mm')
